...
# save the log file
output_path = os.path.join('./logs/', 'forecasting_log')
df_log.to_csv(path_or_buf=output_path + '.csv', index = False)
print('Saved the forecasting_log.csv')

# upload the log file
log_dstore = Datastore(ws, args.datastore)
log_dstore.upload_files(['./logs/forecasting_log'+'.csv'], target_path='forecasting_log_'+str(datetime.datetime.now().date()), overwrite=args.overwrite_logs, show_progress=True)
print('Uploaded the forecasting_log.csv')
