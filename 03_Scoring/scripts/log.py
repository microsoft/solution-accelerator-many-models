...
# 5.0 Read the log file and clean up data
...
output_path = os.path.join('./logs/', 'scoring_log')
df_log.to_csv(path_or_buf = output_path + '.csv', index = False)
print('Saved scoring_log.csv')

# 7.0 Upload the log file
log_dstore = Datastore(ws, args.datastore)
log_dstore.upload_files(['./logs/scoring_log'+'.csv'], target_path = 'scoring_log_' + str(datetime.datetime.now().date()), overwrite = args.overwrite_logs, show_progress = True)
print('Uploaded scoring_log.csv')
